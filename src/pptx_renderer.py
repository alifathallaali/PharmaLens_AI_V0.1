...
from pharmalens.outputs.slide_spec import PresentationSpec
...
def render_pptx(spec: PresentationSpec, output_path: str | Path, *, limiter: UsageLimiter | None = None, theme: str = "default") -> Path:
    limiter = limiter or UsageLimiter.from_env()
    max_slides = int(os.getenv("PPT_MAX_SLIDES_FREE", "6"))
    spec.validate(max_slides=max_slides)
    if not limiter.allow("ppt"):
